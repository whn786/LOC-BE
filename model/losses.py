...
sys.path.append("./wrapper/bilateralfilter/build/lib.linux-x86_64-cpython-38")
from bilateralfilter import bilateralfilter, bilateralfilter_batch

def get_masked_sc_loss(inputs, mask):
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, feat1, feat2, y):
        dist = F.pairwise_distance(feat1.unsqueeze(-1), feat2.unsqueeze(-1).transpose(0, 2)).squeeze()
        #print('dist:',dist.shape)
        y = y.unsqueeze(2).repeat(1, 1, 768)
        loss = torch.mean(
    (y.unsqueeze(1).unsqueeze(2) * torch.pow(dist.unsqueeze(1), 2)) +
    ((1 - y.unsqueeze(1).unsqueeze(2)) * torch.pow(torch.clamp(self.margin - dist.unsqueeze(1), min=0.), 2))
)
        return loss
    
    
def get_aux_loss(inputs, targets):
    inputs = inputs[:, :, :784, :784]
