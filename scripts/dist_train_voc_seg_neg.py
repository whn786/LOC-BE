...
from model.losses import get_masked_sc_loss, get_seg_loss, DenseEnergyLoss, get_energy_loss,ContrastiveLoss,get_aux_loss
from model.model_seg_neg import network
from model.corrloss import ContrastiveCorrelationLoss
...
from utils.camutils import cam_to_label, cam_to_roi_mask2, multi_scale_cam2, multi_scale_cam22, label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi_neg,blur_region,merge_cam_s,mask,cams_to_refine_label,random_crop,MSR
...
parser.add_argument("--data_folder", default='/hy-tmp/VOC2012', type=str, help="dataset folder")
parser.add_argument("--list_folder", default='/hy-tmp/LOC_BE/datasets/voc', type=str, help="train/val/test list file")
parser.add_argument("--num_classes", default=21, type=int, help="number of classes")
parser.add_argument("--crop_size", default=448, type=int, help="crop_size in training")
parser.add_argument("--local_crop_size", default=86, type=int, help="crop_size for local view")
...
parser.add_argument("--w_sc", default=0.3, type=float, help="w_sc")
parser.add_argument("--w_seg", default=0.1, type=float, help="w_seg")
parser.add_argument("--w_reg", default=0.05, type=float, help="w_reg")
parser.add_argument("--w_loc", default=1.0, type=float, help="w_loc")
parser.add_argument("--w_be", default=1.0, type=float, help="w_be")
...
parser.add_argument("--save_ckpt", default='/hy-tmp/LOC_BE/work/checkpoint', help="save_ckpt")
...
def validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            roi_mask_global=cam_to_roi_mask2(_cams.detach(), cls_label=cls_label, low_thre=args.low_thre, hig_thre=args.high_thre)
            #inputs2=blur_region(inputs , roi_mask=roi_mask_global , kernel_size=21,  sigma=5)
            #_cams2, _cams_aux2 = multi_scale_cam2(model, inputs2, args.cam_scales)
            
            
            resized_cam = F.interpolate(_cams, size=labels.shape[1:], mode='bilinear', align_corners=False)
            
            
            _,cls, segs, _,_ ,_= model(inputs,)
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        random_crops = random_crop(inputs, roi_mask, args.local_crop_size, num_crops=4)
        inputs2=mask(inputs , roi_mask,BQD=True)
           
        cam1 = cams
        cam1_aux=cams_aux

        cls1_token,cls, segs1, fmap, cls_aux,attn_weights = model(inputs,hp=False)
        cls2_token,cls2, segs2, fmap2 , cls2_aux,_ = model(inputs2,hp=True)
        clstoken=[]
        for i in random_crops:
            cls_token,_,_,_,_,_=model(i,hp=False)
            clstoken.append(cls_token)
        ...
        cls2_loss=F.multilabel_soft_margin_loss(cls2, cls_label)
        
        # contrastive_loss
        contraloss=[]
        for k in clstoken:
            
            contra_los=contrastive_loss(cls1_token,k,cls_label).cuda()
            contraloss.append(contra_los)            
        contra_loss=sum(contraloss)/4

        # seg_loss & reg_loss
        valid_cam, _ = cam_to_label(cam1.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
        ...
        sc_loss = get_masked_sc_loss(fmap, aff_mask)


        # warmup
        if n_iter <= 2000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_be * cls2_loss  + args.w_sc * sc_loss +  0.0 * seg_loss + 0.0 * reg_loss  
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_sc * sc_loss + args.seg * seg_loss + args.w_reg * reg_loss + args.w_be  * cls2_loss + args.w_loc * contra_loss
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f , cls_loss_aux: %.4f  , cls2_loss:%.4f, sc_loss: %.4f,  seg_loss: %.4f,contra_loss:%.4f  ..." % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'),avg_meter.pop('cls_loss_aux') , avg_meter.pop('cls2_loss'),avg_meter.pop('sc_loss'), avg_meter.pop('seg_loss'),avg_meter.pop('contra_loss')))
