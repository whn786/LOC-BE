...
parser.add_argument("--save_ckpt", default=1 , type=int, help="save_ckpt")
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        inputs2=cachu(inputs , roi_mask,BQD=True)
        ...
        ref_label = cams_to_refine_label(refined_pseudo_label, mask=attn_mask, ignore_index=255)
        aux_loss, pos_count, neg_count = get_aux_loss(attn_weights, ref_label)
        ...
        if n_iter <= 8000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.0 * sc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        elif n_iter <= 12000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.0 * sc_loss + 0.1 * seg_loss + 0.05 * reg_loss
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux +  0.2 * seg_loss + 0.05 * reg_loss + 1.0  * cls2_loss + contra_loss * 1.0 + 0.3 * sc_loss
