...
from utils.camutils import cam_to_label, get_valid_cam, multi_scale_cam2,cam_to_roi_mask2,cachu
...
parser.add_argument("--model_path", default="/hy-tmp/LOC_BE/work_dir_voc_wseg/2023-06-03-17-49-14-720183/checkpoints/model_iter_20000.pth", type=str, help="model_path")
...
parser.add_argument("--list_folder", default='/hy-tmp/ToCo/datasets/voc', type=str, help="train/val/test list file")
...
def _validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            roi_mask_global=cam_to_roi_mask2(_cams.detach(), cls_label=cls_label, low_thre=0.25, hig_thre=0.7)
            #inputs2=blur_region(inputs , roi_mask=roi_mask_global , kernel_size=21,  sigma=5)
            # inputs2=cachu(inputs , roi_mask_global,BQD=False)
            # inputs2  = F.interpolate(inputs2, size=[316, 316], mode='bilinear', align_corners=False)
            inputs3=cachu(inputs , roi_mask_global,BQD=True)
            inputs3  = F.interpolate(inputs3, size=[316, 316], mode='bilinear', align_corners=False)
            #img2 = imutils.denormalize_img(inputs2)[0].permute(1,2,0).cpu().numpy()
            img3 = imutils.denormalize_img(inputs3)[0].permute(1,2,0).cpu().numpy()
            
            _cams2, _cams_aux2 = multi_scale_cam2(model, inputs3, [1.0, 0.5, 1.5])
            
            resized_cam = F.interpolate(_cams, size=labels.shape[1:], mode='bilinear', align_corners=False)
            resized_cam_aux = F.interpolate(_cams_aux, size=labels.shape[1:], mode='bilinear', align_corners=False)
            resized_cam2 = F.interpolate(_cams2, size=labels.shape[1:], mode='bilinear', align_corners=False)
            ...
            imageio.imsave(os.path.join(cam_dir, 'BQD' ,name[0] + ".jpg"), cam_rgb2.astype(np.uint8))
            #imageio.imsave(os.path.join(cam_dir, 'img_QD' ,name[0] + ".jpg"), img2.astype(np.uint8))
            imageio.imsave(os.path.join(cam_dir, 'img_BQD' ,name[0] + ".jpg"), img3.astype(np.uint8))
